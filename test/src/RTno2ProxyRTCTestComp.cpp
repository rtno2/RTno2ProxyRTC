...
/*!
 * @file RTno2ProxyRTCTestComp.cpp
...
{
  RTno2ProxyRTCTestInit(manager);
  RTno2ProxyRTCInit(manager);
  RTC::RtcBase* comp;

  // Create a component
  comp = manager->createComponent("RTno2ProxyRTCTest");
...
bool RunTest()
{
  ...
  comp = mamager.getComponent("RTno2ProxyRTCTest0");
  if (comp == nullptr)
  {
  ...
  RTno2ProxyRTCTest* testcomp = dynamic_cast<RTno2ProxyRTCTest*>(comp);
  if (testcomp == nullptr)
  {
    std::cerr << "Component get failed." << std::endl;
    return false;
