...
update_source_list () {
  ...
  if test "x$rtmsite1" = "x" &&
    ...
    if test "x$kick_shell" = "xn" ; then
      ...
      echo $openrtm_repo | tee /etc/apt/sources.list.d/openrtm.list > /dev/null
      if [ ! -d /etc/apt/keyrings ]; then
        mkdir -p /etc/apt/keyrings
      fi
      wget --secure-protocol=TLSv1_2 --no-check-certificate http://$reposerver/pub/openrtm-keyring.gpg -O /etc/apt/keyrings/openrtm-keyring.gpg
    fi
  elif [ ! -e /etc/apt/keyrings/openrtm-keyring.gpg ]; then
    if test "x$rtmsite1" != "x" ; then
      sed -i '/http:\/\/openrtm.org\/pub\/Linux\/ubuntu\//d' /etc/apt/sources.list
    fi
    if test "x$rtmsite2" != "x" ; then
      rm /etc/apt/sources.list.d/openrtm.list
    fi
    echo $openrtm_repo | tee /etc/apt/sources.list.d/openrtm.list > /dev/null
    wget --secure-protocol=TLSv1_2 --no-check-certificate http://$reposerver/pub/openrtm-keyring.gpg -O /etc/apt/keyrings/openrtm-keyring.gpg
  fi
  ...
  if test "x$fingerprint" != "x" &&
    ...
    sed -i '/https:\/\/packages.fluentbit.io\/ubuntu\//d' /etc/apt/sources.list
  fi
  if [ ! -e /usr/share/keyrings/fluentbit-keyring.gpg ] &&
     test "x$OPT_COREDEVEL" = "xtrue" ; then
    echo $fluent_repo |  tee /etc/apt/sources.list.d/fluentbit.list > /dev/null
    wget -O - https://packages.fluentbit.io/fluentbit.key | gpg --dearmor |  tee /usr/share/keyrings/fluentbit-keyring.gpg >/dev/null
  fi
}
...
#    exit 0
  for p in $*; do
    ...
    if test "x$FORCE_YES" = "xtrue" ; then
       apt install --allow-unauthenticated $p -y
    else
       apt install $p -y
    fi
    if [ $? -ne 0 ]; then
...
install_proc()
{
  ...
  if test "x$arg_python" = "xtrue" ; then
    if test "x$OPT_COREDEVEL" = "xtrue" ; then
      ...
      ret=` python3 -m pip install fluent-logger`
  ...
  if test "x$arg_rtshell" = "xtrue" ; then
    ...
    rtshell_ret=` python3 -m pip install rtshell-aist --break-system-packages`
...
uninstall_proc()
{
  ...
  if test "x$arg_rtshell" = "xtrue" ; then
    ...
    rtshell_ret=` python3 -m pip uninstall -y rtshell-aist rtctree-aist rtsprofile-aist`
    if test "x$rtshell_ret" = "x"; then
